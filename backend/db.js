...
import { deleteFromS3 } from "./s3.js"; // new import
...
export function insertFile(record) {
  db.prepare(`
    INSERT INTO files (id, s3_key, size, uploaded_at, expires_at, download_limit, downloads_done)
    VALUES (@id, @s3_key, @size, @uploaded_at, @expires_at, @download_limit, @downloads_done)
  `).run(record);
}
...
export async function deleteFile(id) {
  ...
  if (rec) {
    try {
      ...
      console.log(`🗑️ Removed from S3: ${rec.s3_key}`);
    } catch (err) {
